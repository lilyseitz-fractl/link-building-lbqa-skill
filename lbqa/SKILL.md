...
- **Percentage groups that should total 100% but don't — always flag.** Whenever a set of percentages is a single-select breakdown that should sum to 100% (a methodology demographic split, a single-choice question's options — e.g. "the gender breakdown was 55% women, 43% men, and 1% who preferred not to say" = 99%), add them up. **If they don't total 100%, flag it for the DJ even when off by a single point.** It's almost always rounding, but the DJ needs to confirm and add a rounding disclaimer ("Percentages may not total 100% due to rounding") if that's the cause. Run this on the WU methodology, any single-select breakdown in the WU body, AND the same breakdowns on the assets. Do NOT flag multi-select / "select all that apply" breakdowns, which exceed 100% by design (a barriers/reasons ranking, etc.) — only single-select sets where the options are mutually exclusive.
...
**Close by explaining the optional next step in plain language — never with the internal term "claim lookups."** The person using this skill may be new to it and won't know what "ready for claim lookups" means, and they may not need it at all. End with something like: *"That's the full pass. Optional next step: as you read through the WU, if you hit any stat or claim you want to double-check, paste it to me and I'll find exactly where it is in the PC — the section or takeaway and the text to search for — and flag anything that doesn't match. Only if it's useful; you may be all set with the fixes above."* Then wait. Don't assume they'll continue.
...
**Output:** a concise **list of edits to make on the asset**, one per discrete issue. **Every asset item must show BOTH the current on-asset text and the corrected text, as a Find → Change to pair** — "Change to: X" alone is confusing because the human can't tell what to locate or what's wrong with it. Use this shape:

- **Find on the asset:** `<the exact current text/value on the graphic>` (name where it sits — "top callout," "heading above the X chart," "Nissan's rank badge") → **Change to:** `<the exact corrected text/value>`
- one brief *Why* clause (the PC value / CM-doc rule). Keep the *Why* for the human's understanding — the thing they actually comment onto the asset is just the **Change to** value.
- For an addition rather than a swap (a missing callout, alt text), use **Add:** `<text>` and name where it goes, instead of Find → Change to.

This is still the "clear-cut fixes get the corrected text, not a lecture" rule (see Posture) — the Find just tells the human what to locate; it is not an explanation of why the rule exists. Use the fuller explanatory image templates (E–I) in `reference/comment-formats.md` only when the DJ/designer must weigh in (an unverifiable stat, a number reconcilable more than one way, anything not settled by the docs). If the asset is clean, say: "✅ This asset matches the PC, follows the CM doc, and has no spelling/grammar errors — no edits needed." Don't invent issues.
...
- **Report only what needs a fix or a decision — never annotate anything that needs no action.** Don't tell the human an element is fine, verified, or correct (e.g. "the subtitle's lowercase 'to' is a preposition, so it's correct — no change"). **This also covers differences that are already resolved and need no action** — e.g. a `$920` vs `$919` gap that the PC's own comment already approved as a round-up. Once you determine something requires nothing from the human, it is NOT a finding: verify it silently and leave it out entirely. Don't "note it just so they know," don't explain why it's fine — that reasoning is exactly the noise the human doesn't want. List only actual fixes and genuine flags (a real, unresolved mismatch, or a decision the human must make). The only place a "clean" signal belongs is the whole-asset / whole-pass verdict ("✅ matches the PC, follows the CM doc — no edits needed"); individual correct or already-resolved elements never get their own line.
